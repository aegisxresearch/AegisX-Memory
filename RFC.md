...
**Amendment (v1.29):** The memory contract stopped depending on the model noticing a rules file, and the client matrix grew from three to seven. (a) **Protocol-level salience.** Both MCP servers now construct `new McpServer` with a server-level `instructions` string — the short contract ("call aegisxmemory_recall at session start… read the coverage comment… save the handoff at session end") — which clients that honor the field inject into the model's context without any file being read; and the tool descriptions in `src/mcp/tools.ts` were rewritten from polite labels into imperatives placed at the point of call: `recall` says "call this BEFORE reading project files", `save` says "call this at session END", `index` says "run once before the first recall on a new repo". This is the layer that works even when no rules file survives, because it travels with the protocol handshake itself. (b) **Four new config writers** in `src/cli/auto-setup.ts`, all reusing the existing JSON merge helper except Codex: **Gemini CLI** (`~/.gemini/settings.json`, `mcpServers`, honoring `GEMINI_CLI_HOME` as a directory — the env override joins the file name rather than being swallowed whole), **Codex CLI** (`~/.codex/config.toml`, honoring `CODEX_HOME`, a zero-dependency TOML writer that only ever *appends* a delimited `[mcp_servers."aegisx-memory"]` table after ensuring a trailing newline — it never rewrites or reorders an existing file, so a config the user customized stays byte-identical ahead of the block; idempotence is a marker-count check), **Windsurf** (`~/.codeium/windsurf/mcp_config.json`, plain `mcpServers`), and **VS Code / Copilot** (`<repo>/.vscode/mcp.json`, which nests under a top-level `servers` key with `type: "stdio"` rather than `mcpServers` — a real schema difference, so it has its own writer and its own doctor reader). `SETUP_AGENTS` is now seven; the wizard menu, `mcp-config --agent` (whose unknown-agent error now lists all of them), and `doctor`'s registration detection all follow the same list. Doctor's per-agent checks now cover the two new formats (VS Code's `servers` nesting, Codex's TOML scan via the bounded `codexEntryIn` reader), and its coverage warning ("installed but not registered in: …") names whichever of the seven lack an entry. Doctor also stopped resolving project-level registrations (`<repo>/.mcp.json`, `<repo>/.vscode/mcp.json`) from `process.cwd()` — they now resolve from the repo passed to `runDoctor`, which is the correct anchor and removes a real bug: running `doctor` from any other directory silently missed a project-level registration that existed. Suite 332 tests (from 321): twelve new writer tests (creation, sibling preservation, idempotence, invalid-JSON refusal, Codex append-only and no-glue-when-no-trailing-newline, env-override directory semantics), three doctor tests made hermetic by mocking `os.homedir` so the machine's real `~/.cursor/mcp.json` from a live install can no longer flip a fixture, the coverage test extended to all seven agents including a real VS Code `servers` registration, and the MCP contract test asserting the `instructions` field and the imperative tool descriptions, so a description regressing to a polite label fails a test.
